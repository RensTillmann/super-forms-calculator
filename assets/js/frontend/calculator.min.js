(function($) { // Hide scope, no $ conflict
	...
	// Init Calculator
	SUPER.init_calculator = function($changed_field, $form, $do_before, $do_after){
        



		// @since 1.5.0 - check if changed field is in a dynamic column, if yes check for regular expression tags



		/*
        // @since 1.7 - able to use regular expressions in {tags}
		var $found = false;
		...
		*/
        ...
        if(typeof $changed_field === 'undefined') {
            ...
            // @since 1.5.0 - first split all the data fields of the calculator field
            $split_fields = $form.find('.super-calculator-wrapper[data-fields*="'+$changed_field.attr('name')+'"]');
            if(typeof $split_fields !== 'undefined'){
            	console.log($split_fields.attr('data-fields'));
            }
            /*
            // First check if index of duplicate column is > 0
            var $column = $changed_field.parents('.super-duplicate-column-fields:eq(0)');
            if($column.index()>0){
				if($column.parent().attr('data-duplicate_dynamically')=='true'){
					// Remove everything behind last dash e.g: "_2"
					var $name = $changed_field.attr('name');
					console.log($name);
					var $position = $name.lastIndexOf('_');
					$name = $name.substring(0, $position != -1 ? $position : $name.length);
					console.log($name);
					$calculator_fields_contain = $calculator_fields.add($form.find('.super-calculator-wrapper[data-fields*="['+$name+'"]'));
